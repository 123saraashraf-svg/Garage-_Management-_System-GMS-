...
// The SupportVehicle class also inherits from the Car class and adds private members for crew size and reliability rating, along with their respective setters and getters. It has a display function that calls the base class's display function and prints the additional details, as well as a Performance function that calculates the performance of the support vehicle based on speed and capacity.
// The saveData function takes vectors of Racers and Support Vehicles and saves their data to a file named "garage.txt". It writes the type of vehicle, followed by its details, for each vehicle in the vectors.
// The loadData function reads the data from the "garage.txt" file and populates the vectors of Racers and Support Vehicles with the loaded data. It checks the type of vehicle and reads the corresponding details into temporary objects, which are then added to the vectors.